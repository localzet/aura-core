...
import (
	...
	"github.com/localzet/aura/common/cmdarg"
...
var cmdRun = &base.Command{
	...
	Short:     "Run Aura with config, the default command",
	Long: `
Run Aura with config, the default command.

The -config=file, -c=file flags set the config files for 
Aura. Multiple assign is accepted.
...
The -test flag tells Aura to test config files only, 
without launching the server.

The -dump flag tells Aura to print the merged config.
	`,
}
...
var (
	configFiles cmdarg.Arg // "Config file for Aura.", the option is customed type, parse in main
	configDir   string
	dump        = cmdRun.Flag.Bool("dump", false, "Dump merged config only, without launching Aura server.")
	test        = cmdRun.Flag.Bool("test", false, "Test config file only, without launching Aura server.")
	 ...
	 */
	_ = func() bool {
		cmdRun.Flag.Var(&configFiles, "config", "Config path for Aura.")
...
func startAura() (core.Server, error) {
